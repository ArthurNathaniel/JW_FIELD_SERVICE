...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new_password !== $confirm_password) {
        ...
        // Verify reset code
        $sql = "SELECT id, reset_code_expiry FROM group_leaders WHERE reset_code = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            if ($current_time <= $expiry_time) {
                ...
                $sql = "UPDATE group_leaders SET password = ?, reset_code = NULL, reset_code_expiry = NULL WHERE reset_code = ?";
                $stmt = $conn->prepare($sql);
